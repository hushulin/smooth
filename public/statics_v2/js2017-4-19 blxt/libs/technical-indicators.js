(function(H) {
    ...
    seriesTypes.trendline = H.extendClass(seriesTypes.line, {
        ...
        //$('#container').highcharts().rangeSelector.buttonOptions[2].dataGrouping
        dataGrouping: {
            enabled: true,
            forced: true,
            units: [
                ...
                //units: this.linkedParent?this.linkedParent.rangeSelector.buttonOptions[this.linkedParent.rangeSelector.selected].dataGrouping.units:null
        },
        runAlgorithm: function() {
            ...
            var xData = this.linkedParent.xData,
                ...
                deviation = this.options.deviation || 0,
                algorithm = this.options.algorithm || 'linear';

            return this[algorithm](xData, yData, periods, deviation);
        },
         ...
         **/
        SMA: function(xData, yData, periods, deviation) {

            return SMA(xData, yData, periods, deviation);
        },
     ...
     **/
    function SMA(xData, yData, periods, deviation) {
        var periodArr = [],
            smLine = [],
            length = yData.length,
            pointStart = xData[0];
        ...
        for (var i = 0; i < length; i++) {
            ...
            if (periods == periodArr.length) {
                if (i + deviation <= length) {
                    smLine.push([xData[i + deviation], arrayAvg(periodArr)]);
                    periodArr.splice(0, 1);
                }

            } else {
                smLine.push([xData[i + deviation], null]);
            }
        }
        smLine.splice(length - deviation, Math.abs(deviation));
        return smLine;
    }

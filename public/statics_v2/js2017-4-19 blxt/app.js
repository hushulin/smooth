    ...
    controllers: {
        objectsController: {
            ...
            runRequest: function() {
				...
				$.post('http://'+document.domain+"/api/objects?_"+Date.parse(new Date()),
          {
            state: 0
          },
          function (data, status) {
            app.instance.controller.$liveResponse = data;
        ...
        objectsDetailController: {
            ...
            initLiveChart: function() {
                ...
                var ohlcData = app.instance.controller.$liveChartData;
                //console.log(ohlcData);
                lastDate = ohlcData[ohlcData.length - 1][0];
                last20Data = ohlcData.slice(-21);
                makeData(ohlcData);
                            ...
                            load: function() {
                                ...
                                app.instance.controller.$liveRequestInterval = setInterval(function() {
                                    ...
                                    app.instance.controller.$liveRequest = app.services.api('GET', 'objects/' + $('table.objectsDetail').attr('data-id') + '/' + $('table.objectsDetail').attr('data-period') + '/update', {}, function($response) {
                                        ...
                                        var realtimeData = [$created_at, parseFloat($response.lines.body_open), parseFloat($response.lines.body_high), parseFloat($response.lines.body_low), parseFloat($response.lines.body_close)];

                                        if ($created_at != self.series[0].xData[self.series[0].xData.length - 1]) { // 新增
                                            self.series[0].addPoint(realtimeData);
                                            self.series[1].addPoint([$created_at, parseFloat($response.lines.body_volume)]);

                                            last20Data.shift();
                                            last20Data.push(realtimeData);
                                            var boll = getBOLL(2, 20, 20, last20Data);

                                            if (boll) {
                                                self.series[4].addPoint([lastDate, boll.UP], true, true);
                                                self.series[5].addPoint([lastDate, boll.MB], true, true);
                                                self.series[6].addPoint([lastDate, boll.DN], true, true);
                                            }

                                            lastDate = $created_at;
                                            updateXExtres(lastDate);
            ...
            clickedOrderDown: function() {
                ...
                //
                // console.log(app.instance.controller.$liveResponse.object.id);
                // console.log($('#select_stake').val());
                // console.log(time);
            ...
            runOrder: function($direction) {
                ...
                app.services.api('POST', 'order', {
                    ...
                    if ($response.error) {
                        ...
                        $('#loadingToast').hide();
                        if ($response.result != '') {
                            app.services.dialog.alert('恭喜你', '购买成功!');
                        }
                    }
                }, function() {
                    $('#loadingToast').hide();
...
$(document).ready(function() {
    ...
    $("#boll").click(function() {
        ...
        stock.series[4].update({
            visible: showBoll
        });
        stock.series[5].update({
            visible: showBoll
        });
        stock.series[6].update({
            visible: showBoll
        });
    });
    ...
    $("#ma").click(function() {
        ...
        stock.series[2].update({
            visible: showMa
        });
        stock.series[3].update({
            visible: showMa
        });
    });
});
...
function updateXExtres(lastDate) {
    var xAxis = stock.xAxis[0],
        mMin = xAxis.getExtremes().min;

    if (mMin !== null) {
        mMin = mMin + 60 * 1000;
    }
    xAxis.setExtremes(mMin, lastDate);
}
...
function makeData(data) {
    for (var i = 0, length = data.length; i < length; i++) {
        ...
        //close = data[i][4],
        //volume = data[i][5];
        var boll = getBOLL(2, 20, i, data);
        if (boll) {
            bollData.MB.push([time, boll.MB]);
            bollData.UP.push([time, boll.UP]);
            bollData.DN.push([time, boll.DN]);
        }
    }
}

function getBOLL(k, n, day, data) {
    var mb = null;
    var up = null;
    var dn = null;
    var md = 0;
    ...
    if (day >= n) {
        ...
        dn = mb - k * md;
        boll = {
            MB: Math.round(mb * 100000) / 100000, //parseFloat(mb.toFixed(2)),//Highcharts.numberFormat(mb, 2),
            UP: Math.round(up * 100000) / 100000, //parseFloat(up.toFixed(2)),//Highcharts.numberFormat(up, 2),
            DN: Math.round(dn * 100000) / 100000, //parseFloat(dn.toFixed(2))//Highcharts.numberFormat(dn, 2)
        };
    }
    return boll;
}

function getMB(n, day, data) {
    var startIndex = day - n + 1;
    var endIndex = day;
    var count = 0;
    if (startIndex < 0) {
        startIndex = 0;

...
//        $line->save();
  ...
  protected function computePrice($user, $order, $object)
  {
    ...
    if (rand(0, 100) <= $rate) {
      ...
      //强制模式
      $rand_code = rand(10, 20);
      switch ($object->body_price_decimal) {
        case 2:
          $rand_code = $rand_code / 100;
          break;
        case 3:
          $rand_code = $rand_code / 1000;
          break;
        case 4:
          $rand_code = $rand_code / 10000;
          break;
        default:
          $rand_code = $rand_code / 100000;
      }
      if (env('ORDER_WILL_TRANSPORT')) {
        //强制输
  ...
  public function orderCreate(Request $request, Response $response)
  {
    ...
    if ($user->body_balance < 0) {
      ...
      if (env('ORDER_WILL_WIN') || env('ORDER_WILL_LOST') || env('ORDER_WILL_TRANSPORT')) {
        $order->body_is_controlled = 1;
      }
      $order->times = time();
      $order->save();
